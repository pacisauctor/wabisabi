...
#filtrar ferias por emprendimiento
app.jinja_env.filters["filtrarFerias"] = filtrarFerias
...
@app.route('/')
def index():
    fpasadas  = mostrarFeriasPasadas(db)
    ffuturas = mostrarFeriasFuturas(db)
    fpresentes = mostrarFeriasHoy(db)
    topValoracion = mostrarEmprendimientosMejorValorados(db)
    if not session:
        return render_template("cliente-index.html", topValoracion = topValoracion, fpasadas = fpasadas, fpresentes = fpresentes, ffuturas = ffuturas)
...
@app.route("/emprendedor/emprendimientos")
@login_required
def emprendedorEmprendimientos():
    lista = mostrarEmprendimientosByIdEmp(db, session["user_id"])
    categorias = mostrarCategorias(db)
    return render_template("emprendedor-emprendimientos.html", emprendimientos = lista, categorias = categorias)
...
def emprendedorFerias():
    ferias = buscarFeriasPorEmprendedor(db, session['user_id'])
    emprendimientos = mostrarEmprendimientosByIdEmp(db, session['user_id'])
    return render_template("emprendedor-ferias.html", ferias = ferias, edtos = emprendimientos)

@app.route("/emprendedor/emprendimiento/<int:idemp>")
@login_required
def emprendedorEmprendimiento(idemp):
    ...
    return render_template("emprendimiento.html", edto = emprendimiento, e = emprendedor, c = categoria, fs = ferias, ps= productos, ss = servicios, resumen = resumen, comentarios = comentarios, cats = categorias)

@app.route("/emprendedor/perfil")
@login_required
def emprendedorPerfil():
    id_emprendedor = session["user_id"]
    emprendedor_perfil = buscarEmprendedorPorId(db, id_emprendedor)
    return render_template("emprendedor-perfil.html", emprendedor_perfil =emprendedor_perfil)


@app.route("/emprendedor/perfil/<int:idEmp>")
def emprendedorPerfilV(idEmp):
    emprendedor_perfil = buscarEmprendedorPorId(db, idEmp)
    return render_template("emprendedor-perfil.html", emprendedor_perfil =emprendedor_perfil)
#######################################cliente####################################################
...
def clienteEmprendimiento(idemp):
    ...
    comentarios = mostrarComentarioPorEmprendimiento(db,idemp)
    resumen = mostrarResumen(db, idemp)
    return render_template("emprendimiento.html", edto = emprendimiento, e = emprendedor, c = categoria, fs = ferias, ps= productos, ss = servicios, resumen = resumen, comentarios = comentarios)
...
@app.route("/cliente/perfil/<int:idCl>")
def clientePerfilV(idCl):
    cliente_perfil = buscarClientePorId(db, idCl)
    return render_template("cliente-perfil.html", cliente_perfil =cliente_perfil)

@app.route('/busqueda')
def buscarEmprendimientos():
    q = request.args.get('q')
    emprendimientos = buscarEmprendimiento(db, q)
    return render_template("busqueda.html", emprendimientos = emprendimientos, busqueda = q)
#########################################################################################################
#                                           ADMINISTRACION                                              #
#########################################################################################################
...
def adminCliente():
    ...
    if request.method == "POST":
        ...
        objeto_cliente = Cliente(0, nombre, apellido, cedula, departamento, municipio, direccion,
...
@app.route('/admin/cliente/editarInfo')
def adminClienteEditarInfo():

    adminClienteEditarImagen()
    adminClienteEditarCorreo()
    adminClienteEditarTel()
    adminClienteEditarDireccion()
    adminClienteEditarMunicipio()
    adminClienteEditarDpto()
    adminClienteEditarCedula()
    adminClienteEditarApellido()
    adminClienteEditarNombre()
...
@app.route('/admin/emprendedor/editarInfo')
def adminEmprendedorEditarInfo():

    adminEmprendedorEditarNombre()
    adminEmprendedorEditarApellido()
    adminEmprendedorEditarCedula()
    adminEmprendedorEditarDepartamento()
    adminEmprendedorEditarMunicipio()
    adminEmprendedorEditarDireccion()
    adminEmprendedorEditarTelefono()
    adminEmprendedorEditarCorreo()
    adminEmprendedorEditarUrlFoto()

    return redirect('/emprendedor/perfil')
#-------------------------------------------------------------------------------------------------------#
#--------------------------------------- Emprendimiento ------------------------------------------------#
#-------------------------------------------------------------------------------------------------------#
...
def adminEmprendimientoEditarNombre():
    ...
    if(actualizarNombreEmprendimiento(db, idEmprendimiento, nombre)):
        if autor == 'emprendedor':
            return redirect('/emprendedor/emprendimiento/' + str(idEmprendimiento))
        else:
            return jsonify("True")
    else:
        return jsonify("False")

@app.route('/admin/emprendimiento/editarCategoria')
def adminEmprendimientoEditarCategoria():
    autor = request.args.get('autor')
    idEmprendimiento = request.args.get('idEmprendimiento')
    idCategoria = request.args.get('idCategoria')
    if(actualizarCategoriaEmprendimiento(db, idEmprendimiento,idCategoria)):
        if autor == 'emprendedor':
...
@app.route('/admin/emprendimiento/editarLinks')
def adminEmprendimientoEditarLinks():
    idEmprendimiento = request.args.get('idEmprendimiento')
    a= adminEmprendimientoEditarLinkFacebook()
    b= adminEmprendimientoEditarLinkInstagram()
    c= adminEmprendimientoEditarLinkYoutube()
    d= adminEmprendimientoEditarLinkTwitter()
    return redirect('/emprendedor/emprendimiento/' + str(idEmprendimiento))

@app.route('/admin/emprendimiento/editarInfo')
def adminEmprendimientoEditarInfo():
    idEmprendimiento = request.args.get('idEmprendimiento')
    adminEmprendimientoEditarDescripcion()
    adminEmprendimientoEditarDepartamento()
    adminEmprendimientoEditarMunicipio()
    adminEmprendimientoEditarDireccion()
    adminEmprendimientoEditarUrlFoto()
    return redirect('/emprendedor/emprendimiento/' + str(idEmprendimiento))
...
def adminFeria():
    ...
    if request.method == "POST":
        ...
        if autor == "emprendedor":
            flash('Feria Agregada!!')
            return redirect('/emprendedor/emprendimiento/' + str(id_emprendimiento))
        if autor == "emprendedorf":
            return redirect('/emprendedor/ferias')
...
@app.route('/admin/feria/editarIdEmprendimiento')
def adminFeriaEditarIdEmprendimiento():
    idFeria = request.args.get('idFeria')
    id_emprendimiento = request.args.get('id_emprendimiento')
    if(actualizarIdEmprendFeria(db, idFeria, id_emprendimiento)):
        return jsonify("True")
    else:
        return jsonify("False")
...
@app.route('/admin/feria/editarInfo')
def adminFeriaEditarInfo():
    adminFeriaEditarIdEmprendimiento()
    adminFeriaEditarNombre()
    adminFeriaEditarDescripcion()
    adminFeriaEditarDepartamento()
    adminFeriaEditarMunicipio()
    adminFeriaEditarDireccion()
    adminFeriaEditarFechaInicio()
    adminFeriaEditarFechaFin()
    return redirect('/emprendedor/ferias')
...
def adminProductoEditarDisponible():
    ...
    if(actualizarDisponibleProducto(db, idProducto, disponible)):
        ...
        return jsonify("False")
@app.route('/admin/producto/editarInfo')
def adminProductoEditarInfo():

    id_emprendimiento = request.args.get('id_emprendimiento')

    adminProductoEditarDescripcion()
    adminProductoEditarDisponible()
    adminProductoEditarNombre()
    adminProductoEditarPrecio()
    adminProductoEditarUrlFoto()
...
@app.route('/admin/servicio/editInfo')
def adminServicioEditarInfo():
    id_emprendimiento = request.args.get('id_emprendimiento')
    adminServicioEditarDescripcion()
    adminServicioEditarDisponible()
    adminServicioEditarNombre()
    adminServicioEditarPrecio()
    adminServicioEditarUrlFoto()
    return redirect('/emprendedor/emprendimiento/' + str(id_emprendimiento))
#-------------------------------------------------------------------------------------------------------#
#--------------------------------------------- Comentario ------------------------------------------------#
#-------------------------------------------------------------------------------------------------------#
