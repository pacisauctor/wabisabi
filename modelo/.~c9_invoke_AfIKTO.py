...
def actualizarNombreCategoria(db, id_categoria, nombre):
    res = db.execute("UPDATE categoria SET nombre=? WHERE id_categoria=?", nombre , id_categoria)
    if res == 0:
        return False
    else:
        return True
...
def insertarCategoria(db, c):
    if type(c) != Categoria:
        return -1
    res = db.execute("INSERT INTO categoria (nombre, descripcion, url_foto) VALUES (:nomb,:descripcion,:urlFoto) ", nomb=c.nombre, descripcion=c.descripcion, urlFoto=c.url_foto)
    return res